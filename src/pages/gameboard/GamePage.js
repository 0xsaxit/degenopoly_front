...
import { setCaseModal, setModal, setPlayerInfo } from '../../actions/manager';
import { claimRewards, getPlayerInfo, hasEnoughPoly, mintNFT, moveTo, rollDice, shortBalance } from '../../lib/block';
...
export default function GamePage() {
  ...
  return (
    <RootStyle title="Arbipoly" id="move_top">
      {/* <Container > */}
      <Stack direction={{ md: 'row', xs: 'column' }} spacing={6} justifyContent='space-between' sx={{ width: '96%', mx: '2%'}}>
        <Stack sx={{ width: { md: '65%', xs: '100%' } }}>
          <Board curPos={player.curPos} onChoose={chooseProperty} />
        </Stack>

        <Stack direction='column' spacing={3} sx={{ width: { md: '35%', xs: '100%' } }}>
          <PlayerInfo />
          ...
          <Stack direction='row' spacing={1} sx={{ width: '100%', border: '1px solid white', padding: 1 }}>
            <LoadingButton loading={rolling} loadingPosition='start' startIcon={<></>} variant='contained' size='large'
              disabled={(!rollable && !rollFree) || rolling} onClick={roll} sx={{ width: '50%', fontSize:'24px', bgcolor: '#859D9A' }}>Roll Dice</LoadingButton>

            <LoadingButton loading={claiming} loadingPosition='start' startIcon={<></>} variant='contained' color='error' size='large' disabled={claiming || Number(player.claimableRewards) === 0} onClick={claim} sx={{ width: '50%', fontSize:'24px', bgcolor: '#859D9A'  }}>Claim Rewards</LoadingButton>
          </Stack>
        </Stack>
      </Stack>
      {/* </Container> */}
